...
| **identity**   | `src/identity/`                            | `createWorkOSResolver` maps Slack user → workforce-directory user via WorkOS Directory Sync, cached in DynamoDB (1h TTL). The raw directory calls + cursor pagination live in the vendored `src/runtime/workos-directory.ts` client; the resolver owns the cache, per-request deadlines, and the fail-soft contract. Bearer-API-key auth — no service-token refresh, no L2 cache                                   |
