...
| **rag**        | `src/rag/`                                 | `createRetriever` runs k-NN (Bedrock Titan embeddings) + BM25 against a narrow `RetrievalBackend` port (null / pgvector / custom adapter), fused via Reciprocal Rank Fusion. `createGenerator` calls Claude Sonnet 5 via Bedrock with a strict system prompt over the verified-accessible documents                                                                                                                     |
...
8.  generate answer (Bedrock Claude Sonnet 5) over the verified-accessible docs
