...
| CC6.3 — Least-privilege access | Pod IRSA role: GetItem/PutItem only (no Scan); Bedrock: specific model ARNs only | IRSA policy on the landing-zone `almanac-platform` role |
...
| CC6.7 — Data encryption at rest | DDB encrypted (AWS-managed KMS); S3 encrypted; token KMS envelope encryption | landing-zone `almanac-platform` substrate |
