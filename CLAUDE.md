...
- **AWS**: `DYNAMODB_TABLE_TOKENS`, `DYNAMODB_TABLE_AUDIT`, `DYNAMODB_TABLE_IDENTITY_CACHE`, `SQS_AUDIT_QUEUE_URL`, `SQS_AUDIT_DLQ_URL`, `KMS_KEY_ID`, `REDIS_URL`. AMP/Tempo/Loki are reached via the grafana-agent (Alloy) receiver in the cluster (eks-gitops), which holds its own Pod Identity association — the app pods carry no observability-backend credentials — see `docs/secrets.md`.
